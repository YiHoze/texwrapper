...
parser.add_argument(
    ...
    action='store_true',
    default=False,
    help='Replace with 자동조사. This option is available '
)
parser.add_argument(
    '-k',
    dest='KeepAux',
...
def build_latex():    
    ...
    if not args.passover:
        if os.path.exists('images'):
            ...
            os.system(cmd)
        for file in glob.glob('blockdiag-*.pdf'):
            cmd = 'pdfcrop.exe %s %s' %(file, file)
            os.system(cmd)
    cmd = 'literal.exe ' + tex # Make long strings breakable.
    os.system(cmd)
    if args.AutoJosa:
