...
\begin{enumerate}
\item \macro{create_epub_directory()} 함수가 \macro[epub]{_epub/} 디렉토리를 만든다. 
그다음에 \macro{create_basic()} 함수가 \macro[mimetype]{_epub/mimetype} 파일과 \macro[container.xml]{_epub/META-INF/container.xml} 파일을 만든다.

\item \macro{copy_additional()} 함수가 \macro{additional} 설정 옵션에 지정된, \macro{cover.xhtml}이나 \macro{basic.css} 같은 파일들을 \macro[OEBPS]{_epub/OEBPS/} 디렉토리로 복사한다.

\item \macro{copy_tex()} 함수가 \macro{tex_files} 설정 옵션에 명시된 파일들을 ---확장자를 .xhtml로 바꾸어--- \macro[OEBPS]{_epub/OEBPS/} 디렉토리로 복사한다.
...
\item \macro{copy_images()} 함수가 \macro{original_image_directory} 설정 옵션에 지정된 디렉토리에서 실제 문서에 삽입되는 이미지 파일들을 \verb{_epub/OEBPS/images/} 디렉토리로 복사한다. 
...
\item \macro{zip_epub()} 함수가 \verb{_epub/} 디렉토리를 압축하여 .epub 파일을 만든다.
\end{enumerate}

\section{스크립트 옵션}

Cloche는 ePub을 새로 만들 때마다 \verb{_epub/} 디렉토리를 비운다.
...
\item[op.py] 이것은 Cloche와 직접적인 관련이 없으나 \verb{wordig.py}가 요구하기 때문에 필요하다.
\item[docenv.conf] 이것은 \verb{op.py}가 참조하는 설정 파일이다. 없어도 무방하다.
\end{macros}
...
하나의 epub 파일에는 다음과 같은 구조의 디렉토리가 압축되어 있다.
...
\begin{macros}
\item[output_directory] 출력 디렉토리를 지정하라. 이 옵션이 지정되지 않으면 \macro[epub]{_epub}으로 설정된다.
...
\item[original_image_directory] 이미지 파일들이 포함된 디렉토리를 지정하라. \titleref{sec:image}\를 보라.
...
소스 파일들이 \verb{_epub/OEBPS/} 디렉토리로 복사될 때 확장자가 \verb{.xhtml}로 바뀐다.
...
이 설정에 의해 \verb{_html/cover.xhtml} 파일과 \verb{_html/basic.css} 파일이 \verb{_epub/OEBPS/} 디렉토리로 복사된다.
...
이 문제를 피하기 위해 \macro{copy_tex()} 함수가 레이텍 파일들을 목적 디렉토리로 복사할 때, \macro{merge_tex()} 함수가 \macro{\input} 명령이 내용에 포함되어 있는지 확인하고, 있으면 해당 줄을 대상 파일의 내용으로 치환한다.
...
따라서 \macro{\graphicspath}를 사용하여 이미지 디렉토리의 경로를 지정하여야 한다.
\begin{code}
\graphicspath{{images/}{images/fallback/}}
\end{code}

\item 설정 파일에 이미지 디렉토리의 경로를 지정하여야 한다.
\begin{code}
